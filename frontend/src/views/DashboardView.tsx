  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 bg-[#0a0f18] border border-white/10 p-3 rounded-sm">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto max-w-md">
            {cameras.map((c) => (
              <button
                ...
                className={`px-2.5 py-1 text-[11px] font-mono-tech font-bold rounded flex items-center gap-1.5 transition-all whitespace-nowrap cursor-pointer ${
        ...
        {/* Live Status Indicators & Register Camera Button */}
        <div className="flex items-center gap-3">
          ...
          {onRegisterFeed && (
            <button
              ...
              className="px-2.5 py-1.5 bg-[#00e5ff] hover:bg-[#00cce6] text-black font-display font-bold text-xs rounded flex items-center gap-1 transition-colors cursor-pointer"
              title="Register New Surveillance Feed"
            >
          ...
          <button
            onClick={refreshAll}
            className="p-1.5 bg-white/5 hover:bg-white/10 text-gray-300 border border-white/10 rounded transition-colors cursor-pointer"
      ...
      <div className="grid grid-cols-12 gap-4">
        {/* Left Column (8 cols): Live Video Feeds */}
          ...
          {displayedCameras.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center p-12 bg-[#090d14] border border-white/10 rounded-sm text-center">
              ...
              <p className="text-xs text-gray-400 font-mono-tech max-w-sm mb-4">
                Add a bundled border video clip, optical webcam, or RTSP stream to start live perception.
              </p>
              <button
                onClick={onRegisterFeed || refreshAll}
                className="px-4 py-2 bg-[#00e5ff] hover:bg-[#00cce6] text-black font-display font-bold text-xs rounded flex items-center gap-1.5 cursor-pointer"
              >
                <Plus className="w-4 h-4" />
                <span>REGISTER SURVEILLANCE FEED</span>
              </button>
            </div>
          )}
        </div>
                    ...
                    {!isAck && (
                      <div className="flex items-center justify-end pt-1">
                        <button
                          ...
                          className="px-2.5 py-1 bg-white/5 hover:bg-white/15 text-gray-300 text-[10px] font-mono-tech font-semibold rounded flex items-center gap-1 border border-white/10 transition-colors cursor-pointer"
                        >
